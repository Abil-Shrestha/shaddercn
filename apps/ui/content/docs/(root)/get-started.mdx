...
This guide provides the essentials for adding **shaddercn** components to your React application.
...
You can install all **shaddercn** primitives at once with a single command:

```bash
npx shadcn@latest add @shaddercn/ui
```
...
```bash
npx shadcn@latest add @shaddercn/ui @shaddercn/colors-neutral
```
