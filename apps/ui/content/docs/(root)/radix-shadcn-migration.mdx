...
description: A comprehensive guide for migrating from Radix UI and shadcn/ui to shaddercn components.
---

This guide is designed for developers who already have applications built with **shadcn/ui** or **Radix UI** and want to adopt **shaddercn** components. **shaddercn is fundamentally built with Base UI from the ground up—it is not an adaptation from Radix UI.** Recognizing that many teams are migrating from Radix-based libraries, this page consolidates all migration instructions for a smooth transition.

## Overview

shaddercn components are built on **Base UI**, not Radix. This means the architecture and API patterns are different by design. However, we've worked to make the migration path as clear as possible by:
...
**Tip:** This migration guide is structured to be LLM-friendly. You can provide this page to AI coding assistants (like Claude, ChatGPT, etc.) to help automate the conversion of your Radix/shadcn components to shaddercn.
...
So, if you want to preserve the original shadcn/ui badge size, you should use the `lg` size in shaddercn.
...
shaddercn button sizes are more compact compared to shadcn/ui, making them better suited for dense applications. We also introduce new sizes (`xs`, `xl`, `icon-sm`, `icon-lg`) for more granular control:

| Size      | Height (shadcn/ui) | Height (shaddercn) |
| --------- | ------------------ | ---------------- |
| `xs`      | -                  | 24px             |
| `sm`      | 32px               | 28px             |
| `default` | 36px               | 32px             |
...
So, for example, if you were using the `default` size in shadcn/ui and you want to preserve the original height, you should use the `lg` size in shaddercn.
...
So, if you want to preserve the original shadcn/ui input height (36px), you should use the `lg` size in shaddercn.
...
shaddercn select sizes are more compact compared to shadcn/ui, making them better suited for dense applications:
...
**Quick Checklist:**
- shaddercn `Slider` uses Base UI's multiple value approach
...
shaddercn toggle group sizes are more compact compared to shadcn/ui, making them better suited for dense applications:
